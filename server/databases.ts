...
class LocalRemoteServerSideDatabaseSyncHolder<Content extends {} = {}> {
  ...
  constructor(
    ...
    this.name = name;
    if (local) this.local = new PouchDB(name, { skip_setup: true, name: name });
  ...
  async sync() {
    if (this.local) {
      const sync = await PouchDB.sync(this.local, this.remote, {
        live: true,
        retry: true,
      });
      return true;
  ...
  static databases = {
    attachments: new LocalRemoteServerSideDatabaseSyncHolder<Attachments>(
      'attachments',
      false,
    ...
    basic: new LocalRemoteServerSideDatabaseSyncHolder<{}>(
      'basic',
      false,
      ['drive team', 'scout', 'pit', 'other', '_admin'],
      ['_admin', 'admin'],
...
for (const database of Object.values(
  LocalRemoteServerSideDatabaseSyncHolder.databases,
)) {
  database.sync();
}

export default {
  LocalRemoteServerSideDatabaseSyncHolder:
    LocalRemoteServerSideDatabaseSyncHolder,
