...
for (let match of props.rowData.rawData) {
  ...
  processorScores.value.push(match.auto.processor + match.teleop.processor || 0);
  matchScores.value.push(match.teleop.net + match.auto.net + match.auto.processor + match.teleop.processor || 0);
}
...
function getAlgaeStats() {
  ...
  let totalTeleop = 0;
  for (let match of props.rowData.rawData) {
    if (match.auto.processor + match.auto.net > maxAuto) maxAuto = match.auto.processor + match.auto.net;
    if (match.teleop.processor + match.auto.net > maxTeleop) maxTeleop = match.teleop.processor + match.teleop.net;

    if (minAuto == null || minAuto > match.auto.processor + match.auto.net) minAuto = match.auto.processor + match.auto.net;
    if (minTeleop == null || minTeleop > match.teleop.processor + match.teleop.net) minTeleop = match.teleop.processor + match.teleop.net;
