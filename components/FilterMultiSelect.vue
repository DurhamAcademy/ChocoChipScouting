    ...
    set(value) {
        for (let option of props.options) {
            ...
            for (let selectedOption of value) {
                if (option.id == selectedOption.id) optionSelected = true
            }
...
if (props.extraOptions) {
    dropdownVariants = ref(
        Array(props.extraOptions.length)
            .fill('')
            .map(() => 'ghost')
    )
}
...
<template>
    <USelectMenu
        ...
        :searchable-placeholder="
            searchPlaceholder != undefined ? searchPlaceholder : 'Search Filters or Add Custom Filter'
        "
    >
        <template #default>
            <UButton color="gray" class="flex-1 justify-between min-h-9">
                <span class="text-slate-600" v-if="value.length == 0">{{
                    filterPlaceholder != undefined ? filterPlaceholder : 'Edit Filters'
                }}</span>
                <div>
                    <UBadge
                        class="mr-1 pl-2.5 pr-2.5"
        ...
        <template #option-empty="{ query }" class="w-full">
            <UButton
                ...
                :label="'add \'' + option + ': ' + query + '\''"
                :variant="dropdownVariants[index]"
                color="gray"
                @mouseover="dropdownVariants[index] = 'soft'"
                @mouseleave="dropdownVariants[index] = 'ghost'"
                @click="addFilter(option, index, query)"
            ></UButton>
        </template>
    </USelectMenu>
</template>
