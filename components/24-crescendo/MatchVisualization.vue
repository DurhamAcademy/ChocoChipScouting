...
let promptedNotesDetailedOptions = [
    ...
    ['Shooing location(s)', 'Ability to avoid defense', 'Weakness of cycles', 'Other'],
            ...
            <div class="flex-auto whitespace-normal max-h-72 w-72 max-w-72">
                ...
                <div class="pb-1">
                    ...
                    <UBadge
                        color="indigo"
                        variant="subtle"
                        v-for="endgame in rowData.rawData[selectedMatch - 1].endgame.endgame"
                        class="mr-1.5 mt-2"
                    >
                        {{ endgame }}
                    </UBadge>
                ...
                <div class="text-wrap max-w-72 h-2/3 max-h-2/3 overflow-y-scroll">
                    <div v-for="(item, index) in rowData.rawData[selectedMatch - 1].notes.promptedNotes">
                        <div v-if="item.selected">
                            <div class="pb-1">
                                ...
                                <UBadge
                                    :color="item.rating > 3 ? 'green' : item.rating < 3 ? 'red' : 'gray'"
                                    :variant="item.rating == 2 || item.rating == 4 ? 'soft' : 'subtle'"
                                    >{{ item.rating }}</UBadge
                                >
                            </div>
                            <div v-for="(text, i) in item.notes">
                                <p v-if="text != ''" class="text-xs pb-0 font-semibold underline-offset-2">
                    ...
                    <UBadge
                        :color="sentimentScore > 1 ? 'green' : sentimentScore < -1 ? 'red' : 'gray'"
                        :variant="sentimentScore < 1 && sentimentScore > -1 ? 'soft' : 'subtle'"
