...
for (let i = 0; i < match.length; i++) {
    ...
    if (currentMatch.matchNumber != -1) {
        let team =
            typeof currentMatch.teamNumber == 'string' ? parseInt(currentMatch.teamNumber) : currentMatch.teamNumber
     ...
     */
        ...
        if (data.length > 0) {
            let arr = {
                ...
                autoAcc: {
                    data: !isNaN(+autoAccuracy(data)[1]) ? Math.round(+autoAccuracy(data)[1] * 1000) / 10 + '%' : '0%',
                    color: '',
                },
                ...
                teleAcc: {
                    data: !isNaN(+teleAccuracy(data)[1]) ? Math.round(+teleAccuracy(data)[1] * 1000) / 10 + '%' : '0%',
                    color: '',
                },
...
*/
    ...
    for (let i = 0; i < teamsData.value.length; i++) {
        teamsData.value[i].team.color = colorify(
            ((sortedTeamPercents.indexOf(teamPercents[i]) + 1) / sortedTeamPercents.length) * 100
        )
        teamsData.value[i].driver.color = colorify(
            calculatePercent(teamsData.value[i].driver.data, Math.min(...data.driver), Math.max(...data.driver))
        )
        teamsData.value[i].defense.color = colorify(
            calculatePercent(teamsData.value[i].defense.data, Math.min(...data.defense), Math.max(...data.defense))
        )
        teamsData.value[i].ampAuto.color = colorify(
            calculatePercent(teamsData.value[i].ampAuto.data, Math.min(...data.ampAuto), Math.max(...data.ampAuto))
        )
        ...
        teamsData.value[i].teleAmp.color = colorify(
            calculatePercent(teamsData.value[i].teleAmp.data, Math.min(...data.teleAmp), Math.max(...data.teleAmp))
        )
        ...
        teamsData.value[i].traps.color = colorify(
            calculatePercent(teamsData.value[i].traps.data, Math.min(...data.traps), Math.max(...data.traps))
        )
...
function colorifyTeam(teamData: TeamTableData, data: DataArrayOrSum) {
    ...
    if (teamData.driver.data != 0) {
        totalPercent +=
            calculatePercent(teamData.driver.data, Math.min(...data.driver), Math.max(...data.driver)) * 0.66
    } else {
        totalPercent +=
            calculatePercent(average(data.driver), Math.min(...data.driver), Math.max(...data.driver)) * 0.66
    }
    if (teamData.defense.data != 0) {
        totalPercent +=
            calculatePercent(teamData.defense.data, Math.min(...data.defense), Math.max(...data.defense)) * 0.33
    } else {
        totalPercent +=
            calculatePercent(average(data.defense), Math.min(...data.defense), Math.max(...data.defense)) * 0.33
    }
    ...
    if (newData)
        return [
            ...
            (successfulAmpCount + successfulSpeakerCount) /
                (missedAmpCount + missedSpeakerCount + successfulSpeakerCount + successfulAmpCount),
                ...
                <div>
                    <table id="teamTable" class="table-auto border-4 border-gray-50">
                        ...
                        <thead class="top-0 sticky bg-gray-50 z-10">
                            ...
                            <tr>
                                <th scope="col" v-for="(col, index) of columns" class="font-medium text-sm">
                                    <UButton
                                        v-if="col.sortable"
                            ...
                            <tr v-for="team of teamsData" class="border-b border-gray-200 dark:border-gray-700">
                                ...
                                <td class="text-center">
                                    <UBadge
                                        ...
                                        variant="soft"
                                        :color="team.defense.data != 0 ? team.defense.color : 'gray'"
                                    />
                                </td>
                                <td class="text-center">
                                    <UBadge :label="team.ampAuto.data" variant="soft" :color="team.ampAuto.color" />
                                </td>
                                <td class="text-center">
                                    <UBadge
                                ...
                                <td class="text-center">
                                    <UBadge :label="team.teleAmp.data" variant="soft" :color="team.teleAmp.color" />
                                </td>
                                <td class="text-center">
                                    <UBadge
                                ...
                                <td class="text-center">
                                    <UBadge :label="team.traps.data" variant="soft" :color="team.traps.color" />
                                </td>
                                <td class="text-center">
                                    <UPopover mode="hover">
                                        <UButton
