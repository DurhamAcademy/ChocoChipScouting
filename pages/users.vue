...
async function signUp() {
    ...
    if (
        !(
            ...
            (sessionRoles.userCtx.roles.includes('_admin') || sessionRoles.userCtx.roles.includes('admin'))
        )
    )
        return
    usersDB.signUp(
...
async function changePassword() {
    ...
    if (
        ...
        return
    usersDB.changePassword(username.value, password.value, {}, function (err, response) {
        if (err) {
            console.log(err)
        } else {
